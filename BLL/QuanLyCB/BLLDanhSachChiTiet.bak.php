<?php
  ...
  function GetNgayHD($id) {
    $sql_hd = "SELECT id, lylich_id, CASE WHEN MAX(ngayhdlaodong) > MAX(ngayhdlamviec) THEN MAX(ngayhdlaodong) ELSE MAX(ngayhdlamviec) END AS 'max' from hopdong  where lylich_id = $id";
  ...
  while ($row_coso = mysql_fetch_array($result_coso)){
    ...
    while ($row_tochuc = mysql_fetch_array($result_tochuc)){
      ...
      while ($row_lylich = mysql_fetch_array($result_lylich)){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.(string)($r), GetNgayHD($row_lylich["id"]));
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.(string)($r), GetNgayHD($row_lylich["id"])); 
