<?php
  ...
  while ($row_coso = mysql_fetch_array($result_coso)){
    ...
    while ($row_tochuc = mysql_fetch_array($result_tochuc)){
      ...
      while ($row_lylich = mysql_fetch_array($result_lylich)){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.(string)($r), date("d-m-Y",strtotime($row_lylich["ngayhopdonglamviec"])));
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.(string)($r), $row_lylich["ngachcongchuc_maso"]);
  ...
  $styleArray = array(
    ...
    unlink("../../file_export/bieu_mau/Danh sach chi tiet can bo online.html");
	  $objWriter->save("../../file_export/Danh sach chi tiet can bo online.html");
	}
  header("Location: ../../file_export/Danh sach chi tiet can bo online.html");
?>
