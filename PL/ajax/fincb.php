<?php 
session_start();
$khuvuc = isset($_REQUEST['khuvuc'])?$_REQUEST['khuvuc']:'';
if(isset($_REQUEST['thu'])){
  ...
  $arrshow=array();
	...
	if($khuvuc==1 || $khuvuc==2 || $khuvuc==3){
	  $sql.= " WHERE  cosodaotao_id='".$khuvuc."'  ";
	}
	else{
	   $sql.= " WHERE 1  "; 
	}
                ...
                <div style:="width:">
                  <table style="border-top-width: 0px; border-right-width: 0px; border-bottom-width: 0px; border-left-width: 0px; border-style: hidden; border-color: initial; width: 100%;">
                    ...
                    while ($row = mysql_fetch_array($result)){
                      ...
                      <td></td>
					  <form method="post">
                      <td><button onClick="abc('.$row['id'].')" type="submit" name="btnXem" id="btnXem'.$row['id'].'" value="'. $row['id'].'">Xem</button></td><td>';
                     if($row['trangthai']==1) $tr.="Đã duyệt"; else $tr.="Chưa duyệt"; 
                     $tr.=' </td>
				
                      <td>
                         <button onClick="abcd('.$row['id'].')" type="submit" name="btnDuyet" id="btnDuyet'.$row['id'].'" value="'.$row['id'].'" >Duyệt</button>
                      </td>
					  	 </form>
                      <td>
